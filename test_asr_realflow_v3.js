...
function audioOnly(seq, bytes) {
  const gz = zlib.gzipSync(bytes);
  const seqBuf = Buffer.alloc(4); seqBuf.writeInt32BE(seq, 0);
  const len = Buffer.alloc(4); len.writeUInt32BE(gz.length, 0);
  return Buffer.concat([header(2,1,0,1), seqBuf, len, gz]);
}
function audioOnlyLast(bytes) {
  const gz = zlib.gzipSync(bytes);
  const len = Buffer.alloc(4); len.writeUInt32BE(gz.length, 0);
  return Buffer.concat([header(2,2,0,1), len, gz]);
}
function parse(buf) {
...
ws.on('message', (data) => {
  ...
  console.log('recv', { ...m, payload: m.payload.slice(0, 180) });
  ...
  if (m.type === 9 && !sent) {
    ...
    for (let off = 0; off < pcm.length; off += chunkSize) {
      ...
      ws.send(isLast ? audioOnlyLast(chunk) : audioOnly(seq, chunk));
      if (!isLast) seq += 1;
    }
