...
ws.on('message', (data) => {
  ...
  if (msgType === 0b1001) { // FullServerResponse
    ...
    if (event === 50) { // ConnectionStarted
      // StartSession with audio format params
      ...
      console.log('📤 StartSession sent with audio format');
      ...
      // 发送音频数据 - 分块发送
      const chunkSize = 6400; // 200ms @ 16kHz 16bit mono = 6400 bytes
      let offset = 0;
      let seq = 0;
      ...
      function sendNextChunk() {
        if (offset >= audioBuffer.length) {
          // 发送最后一包（负包）
          const lastChunk = audioBuffer.slice(audioBuffer.length - chunkSize);
          // flag=3 (NegativeSeq) 表示最后一包
          const lastMsg = createMessage(2, 3, null, null, lastChunk);
          // 设置 sequence 为负值
          const seqBuffer = Buffer.alloc(4);
          seqBuffer.writeInt32BE(-1, 0);
          lastMsg.writeInt32BE(-1, 8); // 在 header 后写入 sequence
          ws.send(lastMsg);
          console.log('📤 Last audio chunk sent (negative seq)');
        ...
        // AudioOnlyClient: msgType=2, flag=1 (PositiveSeq)
        const msg = createMessage(2, 1, null, null, chunk);
        // 写入 sequence
        const seqBuffer = Buffer.alloc(4);
