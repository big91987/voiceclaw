...
ws.on('message', (data) => {
  ...
  if (msgType === 0b1011) { // AudioOnlyServer
    ...
    console.log('🎵 Audio chunk:', payload.length, 'bytes, total:', audioData.length);
    ...
    } else if (event === 351) { // TTSSentenceEnd - 句子结束
...
ws.on('close', () => {
  console.log('🔌 Closed, finished:', finished);
  if (!finished && audioData.length > 0) {
    const audio = Buffer.concat(audioData);
    fs.writeFileSync('/tmp/test_tts_20_output.mp3', audio);
    console.log('✅ Audio saved (on close):', audio.length, 'bytes');
  }
});

setTimeout(() => {
  if (!finished && ws.readyState === WebSocket.OPEN) {
    console.log('⏱️ Timeout, saving audio...');
    if (audioData.length > 0) {
      const audio = Buffer.concat(audioData);
      fs.writeFileSync('/tmp/test_tts_20_output.mp3', audio);
      console.log('✅ Audio saved (timeout):', audio.length, 'bytes');
    }
    ws.close();
  }
}, 15000);
